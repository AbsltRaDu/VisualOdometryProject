...
from src.piplines.pipline import TrainerRNN
...
dataset_train = SequenceDataset(dataset_train, seq=5)
dataset_test = SequenceDataset(dataset_test, seq=5)

train_sampler = ProgressiveWindowBatchSampler(dataset_train, batch_size=2, window_size=WINDOW_SIZE, shuffle=True)
test_sampler = ProgressiveWindowBatchSampler(dataset_test, batch_size=2, window_size=WINDOW_SIZE, shuffle=True)
...
encoder = VisualEncoder(checkpoint_path='/home/aleksandr/Desktop/learning/flownet2-pytorch/checkpoints/FlowNet2-S_checkpoint.pth.tar')
encoder = encoder.to(device)
model = DeepVO(VisualEncoder=encoder)
model = model.to(device)

if os.path.isfile('process_of_fitting/fitting_models/DeepVO.tar'):
...
pipline = TrainerRNN(
    ...
    device=device,
    normalize=normalize,
    name_of_model=os.path.join('process_of_fitting/fitting_models', 'DeepVO.tar'),
    path_to_save_process_of_fitting=os.path.join('process_of_fitting/result_of_fitting', 'DeepVO.json'),
