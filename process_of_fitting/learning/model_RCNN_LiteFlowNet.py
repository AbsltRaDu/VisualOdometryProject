...
model = DeepVO_RAFT(encoder=encoder.encoder, feat_dim=128, hidden_size=256, dropout=0.3)
model = model.to(device)

if os.path.isfile('process_of_fitting/fitting_models/RCNN_LiteFlowNet.tar'):
...
dct_of_results = training_RCNN_progressive_JointTrain_RAFT(train_data, train_data, model, loss_func_pose=loss_func, loss_func_trajectory=loss_func_trajectory, optimizer=optimizer, \
    epochs=epochs, device=device, normalize=None, name_of_model=os.path.join('process_of_fitting/fitting_models', 'RCNN_LiteFlowNet.tar'), \
        path_to_save_process_of_fitting=os.path.join('process_of_fitting/result_of_fitting', 'RCNN_LiteFlowNet.json'), squueze=False, window_size=WINDOW_SIZE, weight_trajectory=0.5)
