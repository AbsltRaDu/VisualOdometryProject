...
dataset_train = mavDatasetCNN_3D('datasets/simulation', transform, normalize=None, device='cpu', lst_of_datasets=lst_of_datasets_for_train, max_size=100) # Сразу формируем все массивы на GPU
dataset_test = mavDatasetCNN_3D('datasets/simulation', transform, normalize=None, device='cpu', lst_of_datasets=lst_of_datasets_for_train, max_size=100)
...
if os.path.isfile('process_of_fitting/fitting_models/RCNNResNet50_VO_SW_TEST.tar'):
    state_dict_cnn = torch.load('process_of_fitting/fitting_models/RCNNResNet50_VO_SW_TEST.tar', map_location=device)
...
dct_of_results = training_RCNN_progressive_JointTrain(train_data, test_data, model, loss_func_pose=loss_func, loss_func_trajectory=loss_func_trajectory, optimizer=optimizer, \
    epochs=epochs, device=device, normalize=None, name_of_model=os.path.join('process_of_fitting/fitting_models', 'RCNNResNet50_VO_SW_TEST.tar'), \
        path_to_save_process_of_fitting=os.path.join('process_of_fitting/result_of_fitting', 'RCNNResNet50_VO_SW_TEST.json'), squueze=False, window_size=WINDOW_SIZE)
