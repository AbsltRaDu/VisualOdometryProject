...
from src.models.modelsNN.model2D.CNN_ResNet18_VO2D import CNN_ResNet18_VO2D
from src.function_of_loss.mse_pose2D import PoseLoss2D, PoseLossTrajectory2D
from src.piplines.pipline import TrainerCNN2D
...
lst_of_dataset_train = lst_of_dataset[-6:-1]
...
model = CNN_ResNet18_VO2D()
model = model.to(device)

if os.path.isfile('process_of_fitting/fitting_models/CNNResNet182D.tar'):
    state_dict_cnn = torch.load('process_of_fitting/fitting_models/CNNResNet182D.tar', map_location=device)
    model.load_state_dict(state_dict_cnn)
    print('Были загружены веса модели с контрольной точки')

epochs = 20
loss_func = PoseLoss2D(k=100)
loss_func_trajectory = PoseLossTrajectory2D()
...
pipline = TrainerCNN2D(
    ...
    device=device,
    normalize=normalize,
    name_of_model=os.path.join('process_of_fitting/fitting_models', 'CNNResNet182D.tar'),
    path_to_save_process_of_fitting=os.path.join('process_of_fitting/result_of_fitting', 'CNNResNet182D.json'),
