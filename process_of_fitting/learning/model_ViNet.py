...
from src.piplines.pipline import TrainerRNNIMU
...
dataset_train = SequenceDatasetIMU(dataset_train, seq=10)
dataset_test = SequenceDatasetIMU(dataset_test, seq=10)
...
deepVO.load_state_dict(state_dict_cnn)

visual_encoder = deepVO.feature_extractor
...
if os.path.isfile('process_of_fitting/fitting_models/ViNet_AirSim.tar'):
...
for param in model.VisualEncoder.conv1.parameters():
...
optimizer = torch.optim.Adam([
    {
        "params": model.VisualEncoder.conv1.parameters(),
        "lr": 1e-6
    },
    {
...
pipline = TrainerRNNIMU(
    ...
    device=device, 
    normalize=None,
    name_of_model=os.path.join('process_of_fitting/fitting_models', 'ViNet_AirSim.tar'),
    path_to_save_process_of_fitting=os.path.join('process_of_fitting/result_of_fitting', 'ViNet_AirSim.json'), 
