...
print(trajectory_fact.positions())
print(trajectory[:-1].positions())

fig = go.Figure()

fig.add_trace(go.Scatter3d(
    x=[x[0] for x in trajectory_fact.positions()],
    y=[x[1] for x in trajectory_fact.positions()],
    z=[x[2] for x in trajectory_fact.positions()],
    mode='lines',
    name='Фактическая траектория'
))

fig.add_trace(go.Scatter3d(
    x=[x[0] for x in trajectory[:-1].positions()],
    y=[x[1] for x in trajectory[:-1].positions()],
    z=[x[2] for x in trajectory[:-1].positions()],
    mode='lines',
