    ...
    def __init__(self, path, transform=None, device='cpu', batchsize=8):
        ...
        for dataset in filter(lambda x: x in ['mav0_easy1'], os.listdir(self.path)):
            ...
            with open(os.path.join(path_local, 'cam1/data.csv'), 'r', encoding='utf-8') as f: # Вытаскиваю словарь камеры 1
                ...
                lst_of_frames = list(zip(frames[:-1], frames[1:]))
                self.pair += lst_of_frames

                start = 0
                lst_of_idx = list(range(start_idx, start_idx+len(lst_of_frames)))
                for i in range(len(lst_of_frames) // self.batchsize):
                    self.batch_groups.append(lst_of_idx[start: start+self.batchsize])
